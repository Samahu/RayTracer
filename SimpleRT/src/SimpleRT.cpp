...
#include <Camera.hpp>
#include "SimpleRT/AntiAliasingFilter.hpp"

Vector3d SimpleRT::color(const Ray3d ray, const CompositeSceneObject& world, int depth) const
{
...
std::vector<Vector3i> SimpleRT::Render(int w, int h, const CompositeSceneObject& world)
{
	...
	auto sampler = RandomAntiAliasingFilter{};

	for (auto y = h - 1; y >= 0; --y)
		for (auto x = 0; x < w; ++x)
		{
			auto c = sampler.Sample(x, y, camera, world, [this](const Ray3d r, const CompositeSceneObject& w) -> Vector3d { return color(r, w, 0); });
